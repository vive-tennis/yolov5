...
class DDRNETSegmentation(torch.nn.Module):
    def __init__(self, model_path, pretrained: bool = False, device: str = "cuda:0"):
        super().__init__()
        self.device = device
        self.model = DDRNet(num_classes=2)
        self.model.load_state_dict(torch.load(str(model_path), map_location='cuda:0'))
